...
- Keep unit tests in `test/ash_agent_studio`, mirroring `lib/` structure with `<filename>_test.exs`, and default to `async: true` when isolation is possible.
...
```
test/
├── ash_agent_studio/           # Unit tests (mirror lib/ structure)
...
defmodule AshAgentStudio.Integration.Metadata.ReqLLM.OpenAITest do
  use AshAgentStudio.IntegrationCase, backend: :req_llm, provider: :openai
