defmodule AshAgentStudio.Observe.Telemetry do
  use GenServer
  ...
  @impl true
  def init(opts) do
    span_table = :ets.new(:ash_agent_studio_span_index, [:set, :public, read_concurrency: true])
    pid_table = :ets.new(:ash_agent_studio_pid_index, [:set, :public, write_concurrency: true])

    config = %{
      span_table: span_table,
      pid_table: pid_table,
      pubsub: Keyword.get(opts, :pubsub, AshAgentStudio.PubSub)
    }
  ...
  defp attach_handlers(config) do
    call_id = "ash_agent_studio-call"
    stream_id = "ash_agent_studio-stream"
    iter_id = "ash_agent_studio-iteration"
    iter_stop_id = "ash_agent_studio-iteration-stop"
    tool_start_id = "ash_agent_studio-tool-start"
    tool_complete_id = "ash_agent_studio-tool-complete"
    tool_decision_id = "ash_agent_studio-tool-decision"
    tool_retry_id = "ash_agent_studio-tool-retry"
    tool_error_id = "ash_agent_studio-tool-error"
    token_id = "ash_agent_studio-token-warning"
    token_progress_id = "ash_agent_studio-token-progress"
    hook_id = "ash_agent_studio-hook"
    progressive_id = "ash_agent_studio-progressive"
    prompt_id = "ash_agent_studio-prompt"
    llm_request_id = "ash_agent_studio-llm-request"
    llm_response_id = "ash_agent_studio-llm-response"
    stream_chunk_id = "ash_agent_studio-stream-chunk"
    call_summary_id = "ash_agent_studio-call-summary"
    stream_summary_id = "ash_agent_studio-stream-summary"
    annotation_id = "ash_agent_studio-annotation"
  ...
  defp broadcast_run(event, run, pubsub) do
    Phoenix.PubSub.broadcast(pubsub, "ash_agent_studio:runs", {event, run})

    if run_id = Map.get(run, :id) do
      Phoenix.PubSub.broadcast(
        pubsub,
        "ash_agent_studio:runs:" <> to_string(run_id),
        {:run_snapshot, run}
      )
    end
  end
